...
const ProjectTile = ({ project, index, position, rotation, activeId, onClick }: ProjectTileProps) => {
  ...
  useEffect(() => {
    ...
    hoverAnimRef.current
      ...
      .to(title.position, { y: hovered ? 0.15 : -0.45 }, 0)
      .to(textBox.position, { y: hovered ? 0.65 : 0.25 }, 0)
      .to(textBox, { fillOpacity: hovered ? 1 : 0, duration: 0.4 }, 0)
      .to(dateGroup.position, { y: hovered ? 2.2 : 1.25 }, 0)
      .to(mesh.scale, { y: hovered ? 1.85 : 1 }, 0)
      .to((mesh as THREE.Mesh).material, { opacity: hovered ? 1 : 0.92 }, 0)
  ...
  const handleClick = (e: ThreeEvent<MouseEvent>) => {
    ...
    gsap.to(button.position, { z: 0, duration: 0.1 }).then(() =>
      gsap.to(button.position, { z: 0.3, duration: 0.3 }),
    );
    ...
    >
      <group ref={projectRef}>
        <mesh>
          ...
          <meshBasicMaterial color="#F7F7F7" transparent opacity={0.92} />
          <Edges color="black" lineWidth={1.5} />
        </mesh>
        <Text
          {...titleProps}
          position={[-1.85, -0.45, 0.101]}
          anchorX="left"
          anchorY="middle"
          maxWidth={3.4}
          fontSize={0.5}
        ...
        <group position={[-1.25, 1.25, 0.01]}>
          <mesh>
            <planeGeometry args={[1.7, 0.4, 1]} />
            <meshBasicMaterial color="#777" opacity={0} wireframe />
            <Edges color="black" lineWidth={1} />
          </mesh>
          <Text {...subtitleProps} position={[-0.7, 0.2, 0]} fontSize={0.3}>
            {project.date.toUpperCase()}
          </Text>
        </group>
        <Text {...subtitleProps} maxWidth={3.8} position={[-1.85, 0.25, 0.1]} fontSize={0.2}>
          {project.subtext}
        </Text>
        {project.url && (
          <group
          ...
          >
            ...
            <Text {...subtitleProps} color="white" position={[-0.4, 0.15, 0.2]} fontSize={0.25}>
              VIEW -&gt;
            </Text>
          </group>
        )}
