...
const reusableLeft = new THREE.Vector3(-0.55, 0, -0.1);
const reusableRight = new THREE.Vector3(0.55, 0, -0.1);
...
const TimelinePoint = ({ point, diff }: { point: WorkTimelinePoint, diff: number }) => {
  ...
  const titleProps = useMemo(() => ({
    ...textProps,
    font: "./Vercetti-Regular.woff",
    fontSize: isMobile ? 0.22 : 0.26,
    maxWidth: isMobile ? 1.9 : 2.2,
    lineHeight: 1.15,
  }), [textProps]);

  const estimatedTitleLines = useMemo(() => {
    const charsPerLine = isMobile ? 18 : 22;
    return Math.max(1, Math.ceil(point.title.length / charsPerLine));
  }, [point.title]);

  const subtitleY = useMemo(() => {
    return -(0.18 + estimatedTitleLines * 0.22 + diff);
  }, [estimatedTitleLines, diff]);

  return (
    <group position={point.point} scale={isMobile ? 0.32 : 0.48}>
      ...
      <group>
        <group position={getPoint}>
          ...
          <group position={[0, -0.5, 0]}>
            ...
            <Text {...textProps} fontSize={0.14} position={[0, subtitleY, 0]} maxWidth={isMobile ? 1.9 : 2.2}>
              {point.subtitle}
            </Text>
          </group>
        </group>
